// Fixed SQLite setup with proper synchronous operations
const sqlite3 = require('sqlite3').verbose();
const path = require('path');

console.log('🚀 Fixed SQLite Setup (Synchronous)');
console.log('===================================\n');
...
function insertSampleData() {
  ...
  `, (err) => {
    ...
    // Wait a bit for data to be inserted, then create indexes
    setTimeout(() => {
      createIndexes();
    }, 1000);
  });
...
function createIndexes() {
  ...
  // Wait a bit for indexes to be created, then verify
  setTimeout(() => {
    verifySetup();
  }, 1000);
}
...
function verifySetup() {
  ...
  db.get('SELECT COUNT(*) as count FROM activities', (err, row) => {
    ...
    console.log('✅ Indexes created');
    console.log('\n🚀 You can now start the server with: node server-sqlite.js');
